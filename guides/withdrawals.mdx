...
| `autoCancel` | number | No | CS2 only. Minutes until undelivered items are cancelled automatically (30-720, default 120, `720` disables). See [Automatic cancellation](#automatic-cancellation). |
...
A successful response only confirms the marketplace accepted the cancel — AssetPay doesn't change the item state inline. The item transitions to `canceled` and your merchant wallet is refunded asynchronously via the usual callback. Refund the user's balance when you receive that `canceled` callback. See the [API reference](/api-reference/trading/cancel-withdraw-item) for the full contract.

<Note>
A cancel lands on `canceled`, not `reverted`. `reverted` means a delivered item came back afterwards (see [Trade Reversals](#trade-reversals)) — a different situation with the same refund shape.
</Note>

## Automatic cancellation

CS2 withdrawals can also cancel themselves. Every withdraw and quick withdraw accepts an optional `autoCancel` field: the number of minutes after creation at which AssetPay cancels anything still undelivered, so an item that never sources does not sit open indefinitely.

| Value | Behaviour |
|-------|-----------|
| omitted | Defaults to `120` (two hours) |
| `30`-`719` | Cancel still-undelivered items after that many minutes |
...
- The marketplace refuses cancels on items younger than 30 minutes, so `autoCancel: 30` fires a few minutes later than the literal deadline. Values below 30 are rejected.
- Only items still undelivered when the timer fires are cancelled. Anything already delivered is untouched, so a partly-filled trade keeps what it received.

Cancelled items behave exactly like a [manual item cancel](#cancelling-a-withdrawal-item): the item moves to `canceled` and the refund reaches your wallet asynchronously through the standard callback. Refund the user's balance when that callback arrives. If the trade was never dispatched to a supplier at all, the whole trade goes to `canceled` and the entire lock is returned in one go.
