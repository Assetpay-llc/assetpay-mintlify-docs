...
| `declined` | **Deposits only.** The end user actively declined the Steam trade offer. No items were received and no credit is applied. |
...
pending ──────► failed   (supplier purchase failed)
    │
    ▼
active ───────► failed   (Steam offer expired / declined)
    │
    ▼
  hold ───────► failed
    │       └──► reverted
...
- **active → failed**: The Steam offer expired or was declined
- **hold → completed**: 7-day hold ended, no reversal detected
- **hold → failed**: Items reversed by Steam during the hold period
...
- **`declined`**: (Deposits only.) The end user declined the trade offer. No items received, no balance settlement.
