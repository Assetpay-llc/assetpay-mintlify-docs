...
Cancelled items behave exactly like a [manual item cancel](#cancelling-a-withdrawal-item): the item moves to `reverted` and the refund reaches your wallet asynchronously through the standard callback. Refund the user's balance when that callback arrives. If the trade was never dispatched to a supplier at all, the whole trade is cancelled and the entire lock is returned in one go.
