...
description: "Cancel a single undelivered item from a merchant self-trade buy (CS2 / Skinshark only)."
---
...
<Note>
CS2 (Skinshark) items only. Rust buys are fulfilled by a polling supplier with no cancel API and cannot be cancelled. Only **self-trade** buys (`source: "self"`) are cancellable through this endpoint.
</Note>
