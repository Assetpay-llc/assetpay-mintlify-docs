...
`canceled` and `declined` items carry no code — the status is the reason. See [Withdrawals → Per-Item Failure Reasons](/guides/withdrawals#per-item-failure-reasons) for handling guidance.
