...
<Note>
CS2 (Assetpay) items only. Rust buys are fulfilled by a polling supplier with no cancel API, and autoseller-sourced items leave nothing to reverse upstream; both return `TRADE_CANCEL_UNSUPPORTED` (44). Only **self-trade** buys (`source: "self"`) are cancellable through this endpoint.
</Note>
...
| 44 | `TRADE_CANCEL_UNSUPPORTED` | The item is Rust or autoseller-sourced, so there is no supplier cancel to call. |
