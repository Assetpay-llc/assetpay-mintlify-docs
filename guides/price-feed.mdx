...
<Note>
The feed requires separately provisioned credentials. If you only need your own priced view of the market (the same items `/secure/market?source=internal` returns, with your fee applied), the WebSocket [Market Updates](/guides/websocket#market-updates) event covers that with just your API key.
</Note>
