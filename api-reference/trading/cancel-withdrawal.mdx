...
<Note>
CS2 (Assetpay) items only. Rust withdrawals and internal-sourced items have no supplier cancel API. If **every** item in the trade is in one of those lanes the call fails with `TRADE_CANCEL_UNSUPPORTED` (44); if only some are, they are simply left out of the result.
</Note>
...
Items the supplier never considered (already delivered, in Steam hold, Rust, internal-sourced) do not appear in `items[]` at all. An item must also be at least **30 minutes old** and not yet terminal; younger items come back as `failed` with `reason: "TRADE_CANCEL_TOO_SOON"`.
